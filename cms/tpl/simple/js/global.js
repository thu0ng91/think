...
function check_login(){
	...
	$.getJSON(url, function(res){
		...
		if(res.status){
			...
			_html += '<a href="' + baseurl + 'index.php?s=login/">登录</a>';
			_html += '<a href="' + baseurl + 'index.php?s=register/">注册</a>';
		}
...
function change_tab(){
	...
	$div_li.mouseover(function(){
		...
		$("div.tab_content>ul").eq(index).show().siblings().hide();
	});

	$("div.tab_content ul").each(function(){
...
function do_search(){
	...
	if(keyword != ""){
		document.location.href	= baseurl + 'index.php?s=book/search/keyword/' + keyword;
	}else{
		alert("请输入书名！");
	}
	return false;
