...
// 显示评分
function show_pingfen(book_id, num, score){
	$("#book_ping_num span").html(num);
	$("#book_score_num").html(score);
	$("#book_score_bar").studyplay_star({ MaxStar: 10, CurrentStar: score, Enabled: true }, 
		function(value){
			do_pingfen(book_id, value);
		}
	);
}

// 书籍评分
function do_pingfen(book_id, score){
	if(show_login()){
		var url = baseurl + 'index.php?s=ajax/score';
		$.getJSON(url, {id: book_id, value: score}, function(res){
			if(res.status){
				show_pingfen(book_id, res.ping_num, res.ave_score);
			}else{
				alert(res.info);
			}
		});
...
function vip_order(){
	var total = 0;
	$(".chapter_order .total").each(function(){
		total += parseInt($(this).find("span").eq(2).html(), 10);
	});
...
var order_data;
var order_total;
// vip章节订阅 type 1: 单章 2：选定章节 3：全部章节
function order_chapter(chap_id, type){
	order_data = ""; order_total = 0;
	if(get_order_data(chap_id, type)){
		var url = baseurl + 'index.php?s=vip/order';
		$.getJSON(url, {data: order_data}, function(res){
			show_res(res);
		});
	}
}
...
// 获取本次订购的vip章节
function get_order_data(chapt_id, type){
	if(type == 2){
		get_vip_checked();
	}else if(type == 3){
		$("#vip_list .total input").attr("checked", true);
		get_vip_checked();
	}else{
		order_data  = chapt_id + ',';
		order_total = parseInt($("#chapter_price").html(), 10);
	}
	if(order_data == ""){
		alert('请选择的要订购的章节');
		return false;
	}else{
		order_data = order_data.slice(0, order_data.length - 1);
		return confirm('本次订购共需 ' + order_total + ' 金币，确认订购吗？');
	}
}

// 获取选中的vip章节
function get_vip_checked(){
	$("#vip_list .total input:checked").each(function(){
		order_data  += $(this).attr("value") + ",";
		order_total += parseInt($(this).parent().parent().find('span').eq(2).html(), 10);
	});
}

// 获取用户信息
function get_user_info(){
	var url = baseurl + "index.php?s=ajax/userinfo";
	$.getJSON(url, function(res){
		if(res.status){
			$(".jb_total").html(res.vip_money);
			if(parseInt($('.cost_total').html(), 10) > parseInt(res.vip_money, 10)){
				$(".order_btn button").eq(2).attr("disabled", true);
			}
		}
	});
}
