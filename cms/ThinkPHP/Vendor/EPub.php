...
class EPub {
	...
	function addChapter($chapterName, $fileName, $chapterData, $autoSplit = false) {
		...
		$chapter = $chapterData;
		if (is_string($chapterData) && mb_strlen($chapterData) > 250000 && $autoSplit) {
			include_once("EPubChapterSplitter.php");
			//vendor('EPubChapterSplitter');
		...
		if (is_string($chapter) && mb_strlen($chapter) > 0) {
	 ...
	 *
	 * Identifier type should only be "URI", "ISBN" or "UUID".
	 *
	 ...
	 */
	function finalize() {
		if ($this->isFinalized || $this->chapterCount == 0 || mb_strlen($this->title) == 0 || mb_strlen($this->language) == 0 || mb_strlen($this->identifier) == 0 || mb_strlen($this->identifierType) == 0) {
			return;
		}
	...
	function sendBook($fileName) {
		...
		if (!headers_sent($headerFile, $headerLine) or die("<p><strong>Error:</strong> Unable to send file <strong>$fileName.epub</strong>. HTML Headers have already been sent from <strong>$headerFile</strong> in line <strong>$headerLine</strong></p>")) {
			if ((ob_get_contents() === false || ($this->ignoreEmptyBuffer && ob_get_contents() == '')) or die("<p><strong>Error:</strong> Unable to send file <strong>$fileName.epub</strong>. Output buffer initialized" . ($this->ignoreEmptyBuffer ? " and contains data" : "") . ".</p>")) {
				...
				header('Content-Length: ' . strlen($this->zipData));
				echo $this->zipData;
			}
		}
	}
	 ...
	 *
	 * @author     Asbjorn Grandt, derived from code by Anis uddin Ahmad <admin@ajaxray.com>
	 * @param      seed    Default 0 which will generate a random UUID, 1 will use the story zip data and MD5 hash to generate a named UUID, anything else (string or integer) will generate a named UUID. 
	...
	function createUUID($seed = 0) {
		...
		if ($seed === 0) {
			$uid = md5(uniqid('', true));
			$variant = substr($uid, 16, 2);
			$variant = dechex((hexdec($variant) & 0x3f) | 0x80);
			$uid = substr($uid, 0, 12) . "4". substr($uid, 13, 3) . $variant . substr($uid, 18, 14);
		} else if ($seed === 1) {
			if ($this->isFinalized) {
				$uid = md5($this->zipData);
				$variant = substr($uid, 16, 2);
				$variant = dechex((hexdec($variant) & 0x3f) | 0x80);
				$uid = substr($uid, 0, 12) . "3". substr($uid, 13, 3) . $variant . substr($uid, 18, 14);
			} else {
